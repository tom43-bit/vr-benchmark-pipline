...
_clap_ckpt_path = "/nfs/xtjin/benchmark/weights/music_speech_audioset_epoch_15_esc_89.98.pt"
LOGFORMAT = "[%(log_color)s%(levelname)-8s%(reset)s]: %(log_color)s%(message)s%(reset)s"
log = logging.getLogger()
device = 'cuda'

def print_variables_memory_extended():
    """打印变量及模型内存占用（更全面）"""
    print("\n" + "="*60)
    print("变量名及内存占用（含模型）")
    print("="*60)
    ...
    for name, var in local_vars.items():
        ...
        try:
            if torch.is_tensor(var):
                mem = var.element_size() * var.nelement() / 1024**2
                device = "GPU" if var.is_cuda else "CPU"
                print(f"{name:<35} {device} {mem:>8.2f} MB")
                total_visible += mem
            elif hasattr(var, 'parameters'):  # 是模型
                mem = sum(p.element_size() * p.nelement() for p in var.parameters()) / 1024**2
                device = "GPU" if next(var.parameters()).is_cuda else "CPU"
                print(f"{name:<35} {device} {mem:>8.2f} MB (模型参数)")
                total_visible += mem
        except:
            pass
    
    print("-"*60)
    print(f"可见变量总计: {total_visible:.2f} MB")
    print(f"系统报告GPU显存: {torch.cuda.memory_allocated()/1024**3:.2f} GB")
    print(f"系统报告GPU缓存: {torch.cuda.memory_reserved()/1024**3:.2f} GB")
    print("="*60)
...
def extract_all_features(video_list,video_dict,video_path,output_cache_path,
                            ...
                            if_ref_video=False,if_ref_audio=False,ref_video_path=None,ref_audio_path=None,):
...
if __name__ == '__main__':
    ...
    #extract_all_features(video_list=video_list,video_dict=video_dict,video_path=video_path,output_cache_path=output_cache_path,if_ref_video=False,if_ref_audio=False)

    num_samples = 1

    gt_cache = Path(os.path.join(output_cache_path,'gt_cache'))
    pred_cache = Path(os.path.join(output_cache_path,'pred_cache'))
