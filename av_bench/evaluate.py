...
from av_bench.utils import (unroll_dict, unroll_dict_all_keys, unroll_paired_dict,
                            unroll_paired_dict_with_key,extract_pred_only,unroll_only_dict)
...
def evaluate(gt_audio_cache: Path, #视频路径
    ...
    print(pred_audio_cache)
    pred_pann_features = torch.load(pred_audio_cache / 'pann_features.pth', weights_only=True)

    """
    if not pred_pann_features:
        print("pann_features 是空字典")
    else:
        print(f"pann_features 不为空，包含 {len(pred_pann_features)} 个样本")
    """
    ...
    if is_paired:
        if if_ref_audio:
            ...
            paired_panns_logits = extract_pred_only(pred_pann_features,num_samples=num_samples)
        ...
        if not skip_clap and (gt_audio_cache / 'clap_laion_text.pth').exists():
            ...
            """
            if not laion_clap_text_features:
                print("laion_clap_text_features 是空字典")
            else:
                print(f"laion_clap_text_features 不为空，包含 {len(laion_clap_text_features)} 个样本")

            if not laion_clap_audio_features:
                print("laion_clap_audio_features 是空字典")
            else:
                print(f"laion_clap_audio_features 不为空，包含 {len(laion_clap_audio_features)} 个样本")
            """
    ...
    if is_paired:
        if if_ref_audio:
            ...
            og.info(f'Unpaired PASST features keys: {unpaired_passt_keys}')
            ...
            pred_passt_logits = unroll_only_dict(pred_passt_logits)
    ...
    if is_paired and paired_laion_clap_text_features is not None:
        ...
        print(paired_ms_clap_text_features.shape,'\n',paired_ms_clap_audio_features.shape)
