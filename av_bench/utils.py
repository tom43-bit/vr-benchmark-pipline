...
# 如果你只需要 Pred 的特征
def extract_pred_only(d: dict, key: str = 'logits', num_samples: Optional[int] = 10):
    """
    只提取 Pred 特征，不依赖 GT
    """
    ...
    return [torch.stack(feat_list, dim=0) for feat_list in paired_feat_list]
...
def unroll_only_dict(
    ...
    if cat:
        return torch.cat(pred_out_list,dim=0)
    else:
        return torch.stack(pred_out_list,dim=0)
