...
import read_csv as my_csv


_syncformer_ckpt_path = "/nfs/xtjin/benchmark/weight/synchformer_state_dict.pth"
_clap_ckpt_path = "/nfs/xtjin/benchmark/weight/music_speech_audioset_epoch_15_esc_89.98.pt"
...
def extract_all_features(video_list,video_dict,video_path,output_cache_path,
    ...
    print("----------------------test--------------------")
    print(video_list)
    video_paths = [os.path.join(video_path,f) for f in video_list if f.endswith('.mp4')]
    print(video_paths)

    #为定义！！！
    audio_length: float = 6 #args.audio_length
    num_workers: int = 32 #args.num_workers
    batch_size: int = 2 #args.gt_batch_size
    ...
    for video in tqdm(video_list):
        ...
        all_laion_clap[video] = text_embed.cpu().squeeze()
        text_embed = ms_clap_model.get_text_embeddings(text_data)
        all_ms_clap[video] = text_embed.cpu().squeeze()
